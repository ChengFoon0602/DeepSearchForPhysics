...
def _normalize_formula_delimiters(text: str) -> str:
    """把 $$...$$、\\(...\\)、\\[...\\] 统一成 $...$（外部渲染器 Typora 等只认 $...$）。

    LLM 对 prompt 规则是概率性服从，这里做确定性兜底：不管模型怎么写，
    落盘前一律归一化，保证生成文件在任何渲染器里公式都能显示。
    """
...
def generate_markdown(
    ...
    try:
        ...
        # 使用 Path 直接写入文本（先归一化公式分隔符，保证跨渲染器兼容）
        file_path.write_text(_normalize_formula_delimiters(content), encoding='utf-8')
